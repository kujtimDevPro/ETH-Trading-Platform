...
const WeekResult = (props) => {
  ...
  return (
      <div className="p4">
        ...
        {
          props.result.length > 0 && props.mode == 1 &&
          props.result.map((el, idx) => (
            <div className={`table_body ${idx < 3? "winner-color" : ""}`} key={idx}>
              ...
              <span className="p4" style={{width: "25%"}}>{`${(parseFloat(el.currentAmount) - parseFloat(el.oldAmount)).toFixed(2)} (${rateLostRaise(parseFloat(el.currentAmount), parseFloat(el.oldAmount))})`}</span>
            </div>
          ))
        }
        ...
        {
          props.result.length > 0 && props.mode == 2 &&
          props.result.map((el, idx) => {
            if(el.is_winner)
              return (
